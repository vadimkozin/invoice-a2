...
const IData = {
  ...
  invoice: IInvoiceOpts, // опции для СФ
}
...
const getTextAttorney = (executer) => `по доверенности № ${executer.attorneyNumber} от ${executer.attrorneyDate}`

// возвращает опции для заполнения документа
const getSpecial = ({ what = 'actBasis', opts, trafficType }) => {
  ...
  switch (what) {
    ...
    case 'invoice': // опции для "особой" СФ
      ...
      invoice.unitCode = trafficTypeSymbol === 'vz' ? '--' : opts.sf.unitCode
...
const prepareData = (data) => {
  ...
  data.forEach((v) => {
    ...
    d.invoice = {
      fioDirector: d.executer.fio,
      fioAccountant: d.accountant.fio,
      signDirector: [getTextAttorney(d.executer)],
      signAccountant: [getTextAttorney(d.accountant)],
      unitCode: '--',
    }
    ...
    if (d.specialOptions) {
      ...
      d.actBasis = getSpecial({ what: 'actBasis', opts, trafficType }).join()
