...
const createAct = (data, nameFile) => {
  const widthArea = 130 // ширина документа
  ...
  doc.text(`Акт № ${data.docNumber} от ${data.date}`, xb, yb)
  doc.setLineWidth(0.5)
  printLine(doc, xb, yb + 2, widthArea)

  doc.setFont('PT_Sans-Web-Regular')

  const operator = pdf.breakString({
    doc,
    str: data.operator.nameShort,
  ...
  const customer = pdf.breakString({
    doc,
    str: `${data.customer.name}, ИНН ${data.customer.INN}, ${data.customer.address}`,
    widthMax: widthArea - nameOffsetX,
    fontSize: 10,
  })
  ...
  // Таблица услуг
  let [x, y] = [xb, next(4) + adding.operator + adding.customer] // начало таблицы
  const width = 130 //  ширина
  ...
  doc.line(x + 70, y, x + 70, y + height)
  doc.line(x + 80, y, x + 80, y + height)
  doc.line(x + 90, y, x + 90, y + height)
  doc.line(x + 110, y, x + 110, y + height)

  const yh = 6 // смещение заголовка таблицы от верха
  doc.text('№', x + 2, y + yh)
  doc.text('Наименование работ, услуг', x + 15, y + yh)
  doc.setFontSize(8)
  doc.text('Кол-во', x + 71, y + yh)
  doc.text('Ед.', x + 83, y + yh)
  doc.text('Цена', x + 97, y + yh)
  doc.text('Сумма', x + 116, y + yh)
  ...
  data.services.forEach((el, i) => {
    ...
    doc.text('1', x + 74, y + dy)
    doc.text('-', x + 84, y + dy)

    // цена, сумма
    pdf.printLeft({ doc, str: el.service.cost.toFixed(2), x: x + 110, y: y + dy, fontSize: 7 })
    pdf.printLeft({
      doc,
  ...
  pdf.printLeft({ doc, str: 'Итого:', x: x + 110, y: yy, fontSize: 7 })
  pdf.printLeft({ doc, str: 'Сумма НДС 20%', x: x + 110, y: yy + 5, fontSize: 7 })
  pdf.printLeft({ doc, str: price.toFixed(2), x: x + width, y: yy, fontSize: 7 })
  pdf.printLeft({ doc, str: nds.toFixed(2), x: x + width, y: yy + 5, fontSize: 7 })

  const serviceCount = data.services.length
  doc.setFontSize(8)
  doc.text(`Всего оказано услуг ${serviceCount}, на сумму ${totalSum} руб`, x, yy + 10)
  doc.setFont('PT_Sans-Web-Bold')
  doc.text(totalSumWords, x, yy + 15)
  ...
  printLine(doc, x, yy + 30, widthArea)

  const executerY = yy + 37 // ИСПОЛНИТЕЛЬ и ЗАКАЗЧИК по Y
  ...
  doc.text('ЗАКАЗЧИК', x + 70, executerY)

  doc.setLineWidth(0.3)
  doc.setFont('PT_Sans-Web-Bold')
  doc.line(x, lineSignatureY, x + 60, lineSignatureY)
  doc.line(x + 70, lineSignatureY, x + widthArea, lineSignatureY)

  doc.setFontSize(6)
  doc.setFont('PT_Sans-Web-Regular')
  const executer = `${data.executer.fio} по Доверенности № ${data.executer.attorneyNumber} от ${data.executer.attrorneyDate}`
  doc.text(executer, x + 2, lineSignatureY + 3)

  const custom = pdf.breakString({ doc, str: data.customer.name, widthMax: 62, fontSize: 8 })
  ...
  doc.setFontSize(8)
  doc.text(custom, x + 70, executerY + offsetCustom)
...
function printLine(doc, x, y, width, widthLine = 0.5) {
