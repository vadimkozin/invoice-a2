 ...
 */
const getCostInWords = (cost, currency = 'RUB') => {
  ...
  if (y > 0) {
    ...
    for (i = 4; i >= 0; i--) {
      if (t[i] > 0) {
        if (names[d[i][100]])
          r +=
            ' ' + (typeof names[d[i][100]] == 'object' ? names[d[i][100]][i] : names[d[i][100]])
        ...
        if (names[d[i][11]])
          ...
          if (names[d[i][10]])
            r +=
              ' ' + (typeof names[d[i][10]] == 'object' ? names[d[i][10]][i] : names[d[i][10]])
          if (names[d[i][0]])
