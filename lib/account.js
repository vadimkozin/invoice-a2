 ...
 */
const createAccount = (data, nameFile) => {
  const op = {
  ...
  pdf.printCenter({
    doc,
    str: data.document.textDocument,
    x: xb,
    y: yc,
    width: op.widthArea,
    fontSize: 10,
  ...
  if (data.isContractPrint) {
    yc += op.tab2.hightRow
    pdf.printText({ doc, text: data.contract.textContractPeriod, x: xb + 2, y: yc - 1.5, fontSize: 8 })
  ...
  let attorney = data.director.textAnnorney
  pdf.printLeft({ doc, str: attorney, x: xb + widthHalf - gapHalf, y: yc - 0.75, fontSize: 8 })

  attorney = data.accountant.textAnnorney
  pdf.printLeft({ doc, str: attorney, x: xb + op.widthArea, y: yc - 0.75, fontSize: 8 })

  doc.save(nameFile)
}

const getTextOrganization = (item) => {
  return `ИНН ${item.inn}, КПП ${item.kpp}, ${item.name}, ${item.address}`
}
