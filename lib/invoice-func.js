...
const getDateDMY = (year, month, day) => `${day}.${month}.${year}`
...
const calculateCountRows = ({ doc, str1, str2, wMax1, wMax2, fontSize }) => {
  ...
  const countRow1 = fn.getCountSymbolInString(string1, '\n') + 1
  const countRow2 = fn.getCountSymbolInString(string2, '\n') + 1
...
const drawHeader = ({ doc, cfg, data }) => {
    ...
    const [nameOperator, nameCustomer, countRowName] = 
      calculateCountRows({doc, str1: operator, str2: data.customer.nameFull, wMax1: 100, wMax2: 85, fontSize: 10})
    ...
    print({doc, label:'Счёт-фактура №', value: data.docNumber, x, y, xline, width: 40, fontSize: 10, center: true})
    text({doc, label:'от', x: x + 90, y, fontSize: 10})
    print({doc, label:'', value: data.date, x: x+102, y, xline: x+102, width: 40, fontSize: 10, center: true, end:'(1)'})
    ...
    // const docShipment = `№ п/п 1 №${data.docNumber} от ${getDateDMY(data.year, data.month, data.day)} г.`
    const docShipment = `№ п/п 1 №${data.docNumber} от ${data.date}`
...
// Cодержание таблицы
const drawTable = ({ doc, cfg, data, currentY, offsetY = 5 }) => {
  ...
  const widthsColumns = [6, 50, 14, 6, 18, 14, 16, 21, 12, 14, 19, 22, 12, 17, 34] // все ширины колонок
  ...
  widthsColumns.forEach((w, i) => {
    pdf.printCenter({doc, str: labelsInColumns[i], x: xt + offset, y: yt + 22.8, width: w, fontSize: 7})
    offset += w
  })
  ...
  // prettier-ignore
  {
    data.services.forEach((it, index) => {      
      const w = widthsColumns
      let offset = 0
      let width = w[0]
      let y = yt + heightHeader + cfg.table.heightData * (index + 1) - 4
      ...
      offset = w[0]; width = w[1]
      const name = pdf.breakString({doc, str: it.service.name, widthMax: width - 1, fontSize})
      ...
      pdf.printCenter({doc, str: data.invoice.unitCode, x: xt + offset, y, width, fontSize})

      offset += w[3]; width = w[4]
      pdf.printCenter({doc, str: '--', x: xt + offset, y, width, fontSize})

      offset += w[4]; width = w[5]
      pdf.printCenter({doc, str: '1.00', x: xt + offset, y, width, fontSize})

      offset += w[5] + w[6]
      const cost = it.service.cost.toFixed(2)
      ...
      offset += w[9] + w[10]
      const nds = it.service.nds.toFixed(2)
      pdf.printLeft({doc, str: nds, x: xt + offset, y, fontSize})

      offset += w[11]
      const sum = it.service.sum.toFixed(2)
      pdf.printLeft({doc, str: sum, x: xt + offset, y, fontSize})
      ...
      const height = heightHeader + cfg.table.heightData * (index + 1)
  ...
  { 
    ...
    let width = w[0]
    const fontSize = 7
    const offsetFinalY = heightHeader + cfg.table.heightData * cfg.countService + 4
    ...
    pdf.printLeft({doc, str: data.totalCost.toFixed(2), x: xt + offset, y, fontSize})
    ...
    pdf.printLeft({doc, str: data.totalCostNDS.toFixed(2), x: xt + offset, y, fontSize})

    offset = totalWidth(w, 0, 11); width = w[11]
    pdf.printLeft({doc, str: data.totalSum.toFixed(2), x: xt + offset, y, fontSize})
    ...
    let offsetX = 0
    const offsetY = heightHeader + cfg.table.heightData * cfg.countService
    let yy = yt + offsetY
  ...
  // currentY
  return yt + heightHeader + cfg.table.heightData * cfg.countService + cfg.table.heightLast
}

// Подписи под таблицей (руководитель, бухгалтер, ..)
const drawBottom = ({ doc, cfg, data, currentY, offsetY = 8 }) => {
  ...
  // const fioDirector = `/ ${data.executer.fio} /`
  // const fioAccountant = `/ ${data.accountant.fio} /`
  // const signDirector = `По доверенности № ${data.executer.attorneyNumber} от ${data.executer.attrorneyDate}`
  // const signAccountant = `По доверенности № ${data.accountant.attorneyNumber} от ${data.accountant.attrorneyDate}`
  // const bottomText = '(подпись)' + ' '.repeat(40) + '(ф.и.о)'

  const fioDirector = `/ ${data.invoice.fioDirector} /`
  const fioAccountant = `/ ${data.invoice.fioAccountant} /`
  const signDirector = data.invoice.signDirector
  const signAccountant = data.invoice.signAccountant
  const bottomText = ' '.repeat(16) + '(подпись)' + ' '.repeat(40) + '(ф.и.о)'
  ...
  text({ doc, label: businessman, x: xt, y: yy + 21, fontSize })
  ...
  Object.keys(signatures).forEach((k) => {
    ...
    text({ doc, label: s[k].sign, x, y: y + 6, fontSize })
  })
}
